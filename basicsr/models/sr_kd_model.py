...
class SR_kdModel(BaseModel):
    """Base SR model for single image super-resolution."""

    def __init__(self, opt):
        super(SR_kdModel, self).__init__(opt)
    ...
    def init_training_settings(self):
        ...
        if train_opt.get('sim_opt'):
            self.cri_sim = build_loss(train_opt['sim_opt']).to(self.device)
        else:
            self.cri_sim = None
        if train_opt.get('adpat_gt_opt'):
    ...
    def optimize_parameters(self, current_iter):
        ...
        if self.cri_sim:
            ...
            for i in range(len(self.output_t)):
                if i!=len(self.output_t)-1:
                    sim_loss = self.cri_sim(gt_list,self.output_s[i],self.output_t[i])*self.sim_w  #scale2 /2
                    l_sim+=sim_loss
                    loss_dict['l_sim_s'] += sim_loss
                else:
                    sim_loss = self.cri_sim(gt_list,self.output_s[i],self.output_t[i])*self.sim_wt #scale2 *2
                    l_sim+=sim_loss
        ...
        if self.cri_adapt:
            ...
            for i in range(len(self.output_t)):
                ...
                # else:
                    adapt_loss = self.cri_adapt[i](self.gt,self.output_s[i],self.output_t[i])*self.ada_w #scale2 /6
                    l_adapt+=adapt_loss
                    loss_dict['l_adapt_s'] += adapt_loss
                else:
                    adapt_loss = self.cri_adapt[i](self.gt,self.output_s[i],self.output_t[i])*self.ada_wt #scale2 /3
                    l_adapt+=adapt_loss
